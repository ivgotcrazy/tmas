...
TcpMonitor<Next, Succ>::TcpMonitor(const LoggerSP& logger) 
	: tcp_switch_(false), observer_mgr_(logger)
{
	...
	GET_TMAS_CONFIG_INT("global.tcp.connection-timeout", conn_timeout_);
}
...
bool TcpMonitor<Next, Succ>::Init()
{
	...
	{
		LOG(ERROR) << "Fail to init tcp observer manager";
		return false;
	}
 ...
 *   描述 创建
 ----------------------------------------------------------------------------*/
template<class Next, class Succ>
TcpConnInfo* TcpMonitor<Next, Succ>::ConstructTcpConnInfo()
{
 ...
 *   描述 创建
 ----------------------------------------------------------------------------*/
template<class Next, class Succ>
void TcpMonitor<Next, Succ>::DestructTcpConnInfo(TcpConnInfo* conn_info)
{
	delete conn_info;
}
 ...
 *   描述 创建
 ----------------------------------------------------------------------------*/
template<class Next, class Succ>
void TcpMonitor<Next, Succ>::ReinitTcpConnInfo(TcpConnInfo* conn_info)
{
 ...
 *   描述 创建
 ----------------------------------------------------------------------------*/
template<class Next, class Succ>
void TcpMonitor<Next, Succ>::AbandonTcpConnection(const ConnId& conn_id)
{
 ...
 *   描述 创建
 ----------------------------------------------------------------------------*/
template<class Next, class Succ>
void TcpMonitor<Next, Succ>::RemoveTcpConnection(const ConnId& conn_id)
{
	boost::mutex::scoped_lock lock(conn_mutex_);
 ...
 *   描述 创建
 ----------------------------------------------------------------------------*/ 
template<class Next, class Succ>
ProcInfo TcpMonitor<Next, Succ>::DoProcessMsg(MsgType msg_type, void* msg_data)
{
 ...
 *   描述 创建
 ----------------------------------------------------------------------------*/
template<class Next, class Succ>
void TcpMonitor<Next, Succ>::ProcessTcpPkt(PktMsg* pkt_msg)
{
	...
	// 在连接中处理报文
	conn_info->processor->ProcessMsg(MSG_PKT, (void*)pkt_msg);
}
 ...
 *   描述 创建
 ----------------------------------------------------------------------------*/
template<class Next, class Succ>
bool TcpMonitor<Next, Succ>::GetTcpConnInfo(const PktMsg* pkt_msg, 
	                                        TcpConnInfoSP& conn_info)
{
	...
	{
		hash_view.modify(hash_iter, LastPktInModifier(pkt_msg->arrive));
		conn_info = *hash_iter;
	...
	conn_info->last_pkt_in = pkt_msg->arrive;
 ...
 *   描述 创建
 ----------------------------------------------------------------------------*/
template<class Next, class Succ>
void TcpMonitor<Next, Succ>::OnTick()
{
	ClearTimeoutConns();
}
 ...
 *   描述 创建
 ----------------------------------------------------------------------------*/
template<class Next, class Succ>
void TcpMonitor<Next, Succ>::RecordConnTimeout(const ConnId& conn_id)
{
	TcpRecordInfo record_info;

	record_info.conn_id = conn_id;
	record_info.conn_event = TCE_CONN_TIMEOUT;

	observer_mgr_.Process(record_info);
}
 ...
 *   描述 创建
 ----------------------------------------------------------------------------*/
template<class Next, class Succ>
void TcpMonitor<Next, Succ>::PrintTcpInfo(const PktMsg* pkt_msg)
{
