 ...
 *   描述 创建
 -----------------------------------------------------------------------------*/
void HttpRunSession::ProcessPacket(const PktMsg* pkt_msg)
{
 ...
 *   描述 创建
 -----------------------------------------------------------------------------*/
void HttpRunSession::SessionInitProc(const PktMsg* pkt_msg)
{
    ...
    session_info_.request_direction = SRC_IP(pkt_msg) < DST_IP(pkt_msg) ?
        PKT_DIR_S2B : PKT_DIR_B2S;
 ...
 *   描述 创建
 -----------------------------------------------------------------------------*/
void HttpRunSession::RecvingRequestHeaderProc(const PktMsg* pkt_msg)
{
 ...
 *   描述 创建
 -----------------------------------------------------------------------------*/
void HttpRunSession::RecvingRequestDataProc(const PktMsg* pkt_msg)
{
	DLOG(INFO) << "Enter HttpRunSession::RecvingRequestHeaderProc";
 ...
 *   描述 创建
 -----------------------------------------------------------------------------*/
void HttpRunSession::RecvedRequestProc(const PktMsg* pkt_msg)
{
 ...
 *   描述 创建
 -----------------------------------------------------------------------------*/
void HttpRunSession::RecvingResponseHeaderProc(const PktMsg* pkt_msg)
{
 ...
 *   描述 创建
 -----------------------------------------------------------------------------*/
void HttpRunSession::RecvingResponseDataProc(const PktMsg* pkt_msg)
{
 ...
 *   描述 创建
 -----------------------------------------------------------------------------*/
void HttpRunSession::RecvedResponseProc(const PktMsg* pkt_msg)
{
...
void HttpRunSession::RecvedCompleteHttpRequest(const char* data, uint32 len)
{
	...
	HttpRequest& http_request = session_info_.request_info;
	...
	// 需要记录请求头信息
	if (!HPN::GetHttpHeader(data, len, session_info_.request_header))
	{
		DLOG(WARNING) << "Fail to get http request header";
		return;
	}
...
void HttpRunSession::RecvedCompleteResponseHeader(const char* data, uint32 len)
{
	...
	if (!HPN::GetHttpHeader(data, len, session_info_.response_header))
	{
		DLOG(ERROR) << "Fail to get http response header";
		return;
	}

	session_info_.status_code = http_response.status_line.status_code;
