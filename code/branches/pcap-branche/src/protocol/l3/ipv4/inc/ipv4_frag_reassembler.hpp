...
{
public:
	Ipv4FragReassembler(Ipv4MonitorType* monitor, const PktMsgSP& pkt_msg);

	void ProcessFragPkt(const PktMsgSP& pkt_msg);
...
private:
	...
	// 插入分片处理
	bool InsertPktFrag(const PktMsgSP& pkt_msg);

	// 处理首分片
	bool ProcFrontPktFrag(const PktMsgSP& pkt_msg);

	// 处理尾分片
	bool ProcRearPktFrag(const PktMsgSP& pkt_msg);

	// 第一个分片到来处理
	void ProcFirstInPktFrag(const PktMsgSP& pkt_msg);

	// 将分片插入到分片列表
	void DoInsertPktFrag(const PktMsgSP& pkt_msg);
