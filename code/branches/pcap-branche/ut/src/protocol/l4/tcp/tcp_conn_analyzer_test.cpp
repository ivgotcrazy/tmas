 ...
 * 描述: 构建Rst报文
 *----------------------------------------------------------------------------*/
PktMsgSP TcpConnAnalyzerTest::ConstructRstPkt()
{
	TcpConstructor tcp_constructor;
	...
	// 设置TCP层参数
	tcp_constructor.SetSeqNum(40002);
	tcp_constructor.SetAckNum(50002);

	tcp_constructor.SetRstFlag(true);
 ...
 * 描述: 构建内容报文
 *----------------------------------------------------------------------------*/
PktMsgSP TcpConnAnalyzerTest::ConstructContentPkt()
{
	DummyConstructor dummy_constructor(40);
	TcpConstructor& tcp_request = *(dummy_constructor.GetTransportLayer());

	SET_PKT_DEFAULT_PARA(tcp_request, DEFAULT_SRC_IP, 
		DEFAULT_SRC_PORT, DEFAULT_DST_IP, DEFAULT_DST_PORT);

	// 设置TCP层参数
	tcp_request.SetSeqNum(40002);
	tcp_request.SetAckNum(50002);
	tcp_request.SetAckFlag(true);

	// 处理报文
	PktMsgSP pkt_msg(new PktMsg());
	dummy_constructor.GetPacket((void**)(&(pkt_msg->pkt.buf)),
								  (size_t*)(&(pkt_msg->pkt.len)));
	// 解析报文
	TMAS_ASSERT(ParsePktInfo(pkt_msg));
 ...
 * 描述: 发送Rst报文
 *----------------------------------------------------------------------------*/
TEST_F(TcpConnAnalyzerTest, Process_Normal_5)
{
	AccomplishTcp3WHS();
	...
	//--- 发送Rst报文
	//--------------------------
	PktMsgSP pkt = ConstructRstPkt();
	
	VoidSP void_pkt = VOID_SHARED(pkt);
	ASSERT_EQ(PI_RET_STOP, tcp_conn_analyzer_->Process(MSG_PKT, void_pkt));
}

#if 0
/*------------------------------------------------------------------------------
 * 描述: 有内容的tcp报文   l4_monitor没有后继为空指针，不能测试通过
 *----------------------------------------------------------------------------*/
TEST_F(TcpConnAnalyzerTest, Process_Normal_6)
{
	AccomplishTcp3WHS();
	...
	//--- 发送Rst报文
	//--------------------------
	PktMsgSP pkt = ConstructContentPkt();
