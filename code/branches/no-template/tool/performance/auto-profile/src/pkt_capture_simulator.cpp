...
PressureTestSimulator::PressureTestSimulator(const PktHandler& handler)
	: stop_flag_(false), pkt_handler_(handler)
{
...
static void ReleasePktInfoStub(PktInfo* pkt_info)
{
	return;
}
...
#define PM_SEND_PACKET(packet, pkt_index)						\
	...
	pkt_info_sp.reset(&pkt_info, ReleasePktInfoStub);			\
	g_prof_stat.start_time = MicroTimeNow();					\
	pkt_processor_->Process(MSG_PKT, VOID_SHARED(pkt_info_sp));	\
	g_prof_stat.stop_time = MicroTimeNow();						\
	CalcTimeUseOfAllLayers(pkt_index);							\
	boost::this_thread::sleep(boost::posix_time::millisec(1));	


ProfMeasureSimulator::ProfMeasureSimulator(const PktProcessorSP& processor)
